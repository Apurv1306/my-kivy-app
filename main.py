...
from kivy.graphics import Color, Line, Rectangle, Quad
...
GOOGLE_FORM_VIEW_URL: str = "https://docs.google.com/forms/u/0/d/e/1FAIpQLScO9FVgTOXCeuw210SK6qx2fXiouDqouy7TTuoI6UD80ZpYvQ/viewform"
...
SMTP_PORT: int = 587
ADMIN_EMAIL_ADDRESS: str = os.environ.get("FACEAPP_ADMIN_EMAIL", "admin@example.com") # <--- REPLACE WITH YOUR ADMIN EMAIL

Logger = print # Simple logger helper
...
def _create_kivy_texture_from_cv_image(cv_image: np.ndarray) -> Texture:
    ...
    """
    h, w = cv_image.shape[:2]
    colorfmt = "luminance" if len(cv_image.shape) == 2 else "bgr"
    buf = cv2.flip(cv_image, 0).tobytes()
    img_texture = Texture.create(size=(w, h), colorfmt=colorfmt)
    img_texture.blit_buffer(buf, colorfmt=colorfmt, bufferfmt="ubyte")
    return img_texture
...
class FaceApp(App):
    ...
    def __init__(self, **kwargs: Any):
        ...
        self.known_faces_dir: str = "" # Set in build()
    ...
    def _load_haar_cascade(self) -> cv2.CascadeClassifier:
        """Loads the Haar cascade classifier, with fallback."""
        ...
        if cascade.empty():
            ...
            if cascade.empty():
                error_msg = f"[ERROR] Failed to load Haar cascade classifier from both '{HAAR_CASCADE_PATH}' and '{fallback_path}'. Please ensure 'haarcascade_frontalface_default.xml' is present and accessible."
                Logger(error_msg)
                raise RuntimeError(error_msg)
            else:
                Logger(f"[INFO] Successfully loaded Haar cascade from fallback: '{fallback_path}'.")
        else:
    ...
    def build(self):
        ...
        self._setup_buttons(root)
        self._setup_camera()
        self._setup_status_label(root)
        self._setup_recognition_preview(root)
        return root

    def _setup_buttons(self, root: FloatLayout) -> None:
        """Sets up the registration and update buttons."""
        button_bar = BoxLayout(
            orientation="horizontal", size_hint=(1, None), height=dp(48),
            pos_hint={"center_x": 0.5, "y": 0.02}, spacing=dp(10), padding=dp(10),
        )
        self.register_btn = Button(text="Register New Face", background_color=(0.13, 0.59, 0.95, 1))
        self.update_btn = Button(text="Update Photos", background_color=(0.20, 0.80, 0.20, 1))
    ...
    def _setup_camera(self) -> None:
        """Initializes the camera and starts the capture thread."""
        ...
        if not self.capture.isOpened():
            ...
            self.capture_thread = threading.Thread(target=self._camera_loop, daemon=True, name="CameraThread")
            self.capture_thread.start()
            Clock.schedule_interval(self._update_texture, 1 / 30)

    def _setup_status_label(self, root: FloatLayout) -> None:
        """Sets up the status message label."""
        self.status_label = Label(
            text="", size_hint=(None, None), size=(dp(400), dp(50)),
            pos_hint={"center_x": 0.5, "top": 0.95}, color=(1, 1, 0, 1),
            font_size=dp(20), bold=True, halign='center', valign='middle'
        )
        root.add_widget(self.status_label)

    def _setup_recognition_preview(self, root: FloatLayout) -> None:
        """Sets up the temporary recognition display."""
        self.preview_layout = BoxLayout(
            orientation='vertical', size_hint=(None, None),
            size=(dp(260), dp(390)), pos_hint={'x': 0.02, 'center_y': 0.5},
            padding=dp(10), spacing=dp(5), opacity=0,
        )
    ...
    # ------------------------------------------------------------------
    @staticmethod
    def _update_btn_border(instance, *_) -> None:
    ...
    def _update_preview_canvas(self, instance, value) -> None:
    ...
    def _show_popup(self, title: str, content_widget: Any, *, size=(0.8, 0.5)) -> Popup:
        ...
        main_content_layout.add_widget(content_widget)

        back_button = Button(
            text="Back to Camera", size_hint=(1, None), height=dp(40),
    ...
    def _show_status_message(self, message: str, duration: float = 3.0, color=(1, 1, 0, 1)) -> None:
        """Displays a temporary status message on the UI."""
        Clock.schedule_once(lambda _dt: self._set_status_label(message, color, duration), 0)

    def _set_status_label(self, message: str, color: Tuple[float, float, float, float], duration: float) -> None:
        """Helper to set status label text and color, then clear after duration."""
        self.status_label.text = message
        self.status_label.color = color
        Clock.schedule_once(lambda _dt: self._clear_status_message(), duration)
    ...
    def _flash_image_widget(self) -> None:
        ...
        with self.image_widget.canvas.after:
            Color(0, 1, 0, 1)
            self.flash_rect = Line(width=3, rectangle=(self.image_widget.x, self.image_widget.y, self.image_widget.width, self.image_widget.height))
    ...
    def _camera_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            for (x, y, w_s, h_s) in faces:
                ...
                exp_w, exp_h = int(w_full * expansion_factor), int(h_full * expansion_factor)
                center_x, center_y = x_full + w_full // 2, y_full + h_full // 2
                exp_x, exp_y = center_x - exp_w // 2, center_y - exp_h // 2
                ...
                label, conf = self._predict_face(grayscale_face_roi)
                name, emp_id = self.label_map.get(label, ("unknown", ""))
                now = time.time()

                if conf < 60:
                    self._handle_recognized_face(name, emp_id, color_face_roi, now, x_full, y_full, w_full, h_full)
                else:
                    self._draw_face_rectangle(frame, x_full, y_full, w_full, h_full, (0, 0, 255), "Unknown")

            self._put_frame_in_queue(frame)

    def _predict_face(self, face_roi: np.ndarray) -> Tuple[int, float]:
        """Performs face prediction and handles potential errors."""
        try:
    ...
    def _handle_recognized_face(self, name: str, emp_id: str, color_face_roi: np.ndarray, now: float, x_full: int, y_full: int, w_full: int, h_full: int) -> None:
        """Handles actions for a recognized face."""
        ...
        self._draw_face_rectangle(frame, x_full, y_full, w_full, h_full, (0, 255, 0), f"{name.title()} ({emp_id})")
        self._overlay_tick_next_to_name(frame, x_full, y_full - 10, name.title(), emp_id, 0.7, 2)

    def _draw_face_rectangle(self, frame: np.ndarray, x: int, y: int, w: int, h: int, color: Tuple[int, int, int], text: str) -> None:
        """Draws a rectangle and text label on the face."""
        cv2.rectangle(frame, (x, y), (x + w, y + h), color, 2)
        cv2.putText(frame, text, (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)

    def _put_frame_in_queue(self, frame: np.ndarray) -> None:
        """Puts the latest frame into the queue, discarding older ones."""
    ...
    def _update_texture(self, _dt) -> None:
        ...
        self.image_widget.texture = _create_kivy_texture_from_cv_image(frame)

    # ------------------------------------------------------------------
    # Training / retraining recogniser
    # ------------------------------------------------------------------
    def _train_recognizer(self) -> Tuple[Any, Dict[int, Tuple[str, str]]]:
        ...
        for file in sorted(os.listdir(self.known_faces_dir)):
            ...
            images.append(cv2.resize(img_gray, (200, 200)))
    ...
    # ------------------------------------------------------------------
    def _register_popup(self, _btn) -> None:
        ...
        def _submit(_):
            ...
            popup.dismiss()
            threading.Thread(
                target=self._capture_samples, args=(name, emp_id, False), daemon=True, name="CaptureSamples(New)",
            ).start()
        submit_btn.bind(on_press=_submit)

    def _update_photos_popup(self, _btn) -> None:
        ...
        def _next(_):
            ...
            name_existing = next((nm for _lbl, (nm, eid) in self.label_map.items() if eid == emp_id), None)
            popup.dismiss()
    ...
    # ------------------------------------------------------------------
    def _email_registration_flow(self, emp_id: str, name: Optional[str]) -> None:
    ...
    def _send_otp_flow(self, emp_id: str, email: str, name: Optional[str] = None) -> None:
        ...
        def _send_thread():
            ...
            admin_mail_ok = self._send_otp_email(ADMIN_EMAIL_ADDRESS, otp, resolved_name, emp_id, is_admin_email=True)

            Clock.schedule_once(lambda _dt: sending_popup.dismiss())
            if user_mail_ok:
                Clock.schedule_once(lambda _dt: self._otp_verify_popup(emp_id, email))
            else:
                Clock.schedule_once(lambda _dt: self._show_popup("Error", Label(text="Failed to send OTP email. Please check console/internet connection."), size=(0.7, 0.4)))
    ...
    def _otp_verify_popup(self, emp_id: str, email: str) -> None:
        ...
        resend_btn = Button(text="Resend", size_hint=(1, None), height=dp(40)) # Renamed from resend_widget

        content.add_widget(Label(text=f"OTP sent to {email}"))
        content.add_widget(otp_input)
        content.add_widget(verify_btn)
        content.add_widget(resend_btn) # Use resend_btn here
        ...
        def _verify(_):
            if otp_input.text.strip() == self.otp_storage.get(emp_id):
                ...
                threading.Thread(
                    target=self._capture_samples, args=(name_for_capture, emp_id, True, 5), daemon=True, name="CaptureSamples(Update)",
                ).start()
            else:
                otp_input.text = ""
        ...
        def _resend(_):
            ...
            self._send_otp_email(email, new_otp, name_for_resend, emp_id, is_admin_email=False)
            self._send_otp_email(ADMIN_EMAIL_ADDRESS, new_otp, name_for_resend, emp_id, is_admin_email=True)
            otp_input.text = ""
    ...
    # ------------------------------------------------------------------
    def _capture_samples(self, name: Optional[str], emp_id: str, updating: bool = False, sample_count: Optional[int] = None) -> None:
        ...
        count_target = sample_count or SAMPLES_PER_USER
        existing_files = glob.glob(str(Path(self.known_faces_dir) / f"{name}_{emp_id}_*.jpg"))
        start_index = len(existing_files)
        collected = 0

        Logger(f"[INFO] Starting sample capture for {emp_id} – target {count_target} faces (updating={updating}).")
        self._run_capture_countdown()
        ...
        while collected < count_target and not self._stop_event.is_set():
            ...
            if faces.size > 0:
                x, y, w, h = faces[0]
                face_img_resized = cv2.resize(gray[y : y + h, x : x + w], (200, 200))
        ...
        status_msg = "Face updated!" if updating else "Registration completed!"
        Clock.schedule_once(lambda _dt: self._show_status_message(status_msg, 3, (0, 1, 0, 1)), 0)

    def _run_capture_countdown(self) -> None:
        """Runs the countdown before face capture begins."""
    ...
    def _get_latest_frame(self) -> Optional[np.ndarray]:
        """Gets the freshest frame from the queue, handling camera availability."""
        if not self.capture or not self.capture.isOpened():
    ...
    def _handle_successful_recognition(self, name: str, emp_id: str, face_roi_color: np.ndarray) -> None:
        ...
        threading.Thread(
            target=self._submit_to_google_form, args=(name, emp_id), daemon=True, name="GoogleFormSubmitter",
        ).start()

    def _display_recognition_info(self, name: str, emp_id: str, current_time: str, processed_face_image: np.ndarray) -> None:
        """Displays the recognized person's photo, name, and time on the UI."""
        if all([self.preview_layout, self.preview_image, self.preview_name_label, self.preview_time_label]):
            self.preview_image.texture = _create_kivy_texture_from_cv_image(processed_face_image)
    ...
    def _hide_recognition_info(self, animation, widget) -> None:
        """Fades out the recognition display and clears the image."""
        hide_anim = Animation(opacity=0, duration=0.3)
        hide_anim.start(widget)
        Clock.schedule_once(lambda _dt: self._clear_image_texture(), animation.duration)

    def _clear_image_texture(self) -> None:
        """Clears the preview image texture."""
